...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    *(x-coordinate before action - x-coordinate after action)/dt*. *dt* is the time
    ...
    ```
    env = custom_gym.make('Ant-v2')
    ```

    v3 and beyond take custom_gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = custom_gym.make('Ant-v3', ctrl_cost_weight=0.1, ...)
    ```
    ...
    * v3: support for custom_gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc. rgb rendering comes from tracking camera (so agent does not run away from screen)
