...
from custom_gym.envs.__relocated__ import internal_env_relocation_map
...
class EnvSpecTree(MutableMapping):
    ...
    def _assert_version_exists(
        ...
        # If this version doesn't exist and there only exists a default version
            ...
            message += (
                f'You can initialize the environment as `custom_gym.make("{latest_spec.id}")`.'
            )
            raise error.DeprecatedEnv(message)
        ...
        if ":" in path:
            ...
            try:
                ...
                raise error.Error(
                    ...
                    "make sure the package is installed with `pip install` before calling `custom_gym.make()`"
                )
        else:
            id = path
        ...
        try:
            ...
            if unversioned_spec and spec.version is not None:
                ...
                message = (
                    ...
                    "behavior is that the `custom_gym.make` with the unversioned "
...
def load_env_plugins(entry_point: str = "custom_gym.envs") -> None:
